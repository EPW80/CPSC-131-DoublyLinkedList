		...
		class DoublyLinkedList
		{
			public:
				...
				Iterator insert_after(Iterator pos, const T& value) {
					...
					if (size_ == 0) {
						// List is empty, create new node at head
						head_ = new Node<T>(value);
						tail_ = head_;
						size_++;
						return Iterator(head_, head_, tail_);
						} else if (current_node == nullptr) {
							// Iterator is null, insert at tail
				...
				void reverse() {
					if (size_ > 1) {
						...
						while (current != nullptr) {
							// Push the value of the current node to the front of the temp_list
        				...
        				// Detach the temp list nodes from the temp_list object
        				// to prevent them from being deleted when temp_list goes out of scope
